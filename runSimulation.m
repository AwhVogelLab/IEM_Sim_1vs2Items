function runSimulation(nSamps,ss1_amp,ss2_amp,ss1_noise,ss2_noise,dat_tuningwidth)
...
% ss2_amp: same for ss1 condition
% ss1_noise: sd of gaussian noise applied to each "electrode" in SS1 condition
% ss2_noise: same for ss2 condition
...
fname = ['Sim1vs2_',num2str(nSamps),'Samps_SS1amp',num2str(ss1_amp),'_SS2amp',num2str(ss2_amp),'_SS1noise',num2str(ss1_noise),'_SS2noise',num2str(ss2_noise),'_DatTuning',num2str(dat_tuningwidth),'.mat'];
...
for s = 1:nSamps
    fprintf('Sample = %d\n',s);
    tic
    [pval(s) tstat(s) mn_diff(s) sem_diff(s) mn_ss1(s) sem_ss1(s) mn_ss2(s) sem_ss2(s) mn_ctf_ss1(s,:) mn_ctf_ss2(s,:)] = simulateSample(nSubsPerSamp,ss1_amp,ss2_amp,ss1_noise,ss2_noise,dat_tuningwidth,0);
    toc 
end
...
sim.ss1_amp = ss1_amp;
sim.ss2_amp = ss2_amp;
sim.ss1_noise = ss1_noise;
sim.ss2_noise = ss2_noise;
